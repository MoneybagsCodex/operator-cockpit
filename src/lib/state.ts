...
export const STATE_DIR = process.env.OPERATOR_STATE_DIR || path.join(process.env.HOME || '~', '.operator-state');
...
export function writeEvent(event: AgentEvent & { serializedAt?: string; source?: string }): void {
  const ts = new Date().toISOString().replace(/[:.]/g, '-');
  const filename = `${ts}-${event.id}.json`;
  writeJsonFile(path.join(PATHS.events, filename), {
    ...event,
    serializedAt: event.serializedAt || new Date().toISOString(),
